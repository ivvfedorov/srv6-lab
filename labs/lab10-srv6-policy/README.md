...
IGP всегда ведёт трафик кратчайшим путём. А если этот путь перегружен, а соседний
линк простаивает? Обычный IGP не даёт ответа — нужно менять метрики, что влияет
на весь трафик. SR Policy решает это иначе.

Классический IGP — это Destination-based routing: путь определяется метриками
транзитных узлов. Каждый маршрутизатор принимает независимое решение на основе
своего LSDB и shortest-path алгоритма. Трафик всегда идёт по пути с наименьшей
суммарной метрикой.

SR Policy реализует парадигму Source Routing: headend-маршрутизатор навязывает
пакету явный Segment List, игнорируя IGP-метрики транзитных узлов. Транзитные
узлы выполняют указанные SID, не принимая самостоятельных решений о пути.
В MPLS это делалось через RSVP-TE, в SRv6 — через инкапсуляцию с SRH.

```text
Обычный пакет:                 [IPv6 DA=r3] [ICMPv6]
Пакет с SR Policy:  [IPv6 DA=r2:End.X] [SRH: r3:End, SL=1] [ICMPv6]
                    \_____________внешний заголовок__________/  \_оригинал_/
```

Главная разница: обычный ping к r3 идёт по IGP без SRH. Policy-маршрут создаёт
