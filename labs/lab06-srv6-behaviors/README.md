...
локальному SID. В FRR для этой лаборатории важны два автоматических behavior: `uN` как узловой
End SID и `uA` как adjacency End.X SID. `uN` привязан к locator'у узла, а `uA` привязан к
конкретному интерфейсу и next-hop.

Разница между ними видна в контексте SID: `isis(0)` указывает на узловой SID, а `interface ethX`
