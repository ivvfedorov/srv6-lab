...
После выполнения студент должен уметь объяснить, как BGP переносит клиентский маршрут вместе
...
4. Студент понимает различие между transport locator и service SID.
...
Эта схема является главным академическим результатом ЛР11. Если она заполнена правильно,
студент понимает не только команды FRR, но и путь пакета через transport и service layers.
