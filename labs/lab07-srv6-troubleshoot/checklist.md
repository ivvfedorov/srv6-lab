...
| Encap route | `ip -6 route show \| grep seg6` | Headend encap установлен | Проверить Сценарий 10 route add |
...
| ping ok, no SRH | normal IPv6 forwarding, no encap policy | `ip -6 route show \| grep seg6` | Для SRH нужен Сценарий 10 encap route |
| ping fail after SRv6 | missing kernel SID; seg6 disabled | `sysctl seg6_enabled`, `grep seg6local` | Включить sysctl, проверить zebra |
| BGP VPN route есть, ping VRF fail | service SID/VRF lookup broken | `show ipv6 route vrf TENANT_A`, `grep End.DT6` | Проверить Сценарий 11 config |
