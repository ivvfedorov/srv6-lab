# Сценарий 7: Troubleshooting SRv6
...
После выполнения необходимо уметь строить диагностическую цепочку от симптома к root cause,
...
диагностика идёт послойно, от нижнего уровня к верхнему, чтобы гарантировать
изоляцию уровня отказа.
...
Порядок диагностики — это не формальность, а страховка от ложных выводов.
Послойный подход гарантирует, что каждый следующий шаг опирается на доказанно
работоспособный предыдущий:

| Шаг | Вопрос | Команды |
|-----|--------|---------|
| 1 | Среда исполнения жива? | `containerlab inspect`, `docker ps` |
| 2 | Транспорт (интерфейсы, L1/L2) поднят? | `ip -6 -br addr`, `ip link` |
| 3 | Control Plane: IS-IS соседство поднято? | `show isis neighbor` |
| 4 | Control Plane: Locator/SID видны в FRR? | `show segment-routing srv6 locator/sid` |
| 5 | Data Plane: Kernel получил SRv6 state? | `ip -6 route show table all \| grep seg6` |
| 6 | Трафик реально идёт? | `ping6`, `traceroute6`, `tcpdump` |

Запрещённый анти-паттерн для фиксации результатов: «не работало, перезапустил, заработало».
