...
Сравнение инструментов как средств наблюдения за зданием:
- `vtysh` — архитектурный план: показывает, как маршруты ДОЛЖНЫ быть построены.
- `tcpdump` — камера у входа: фиксирует, какие пакеты зашли и вышли.
- `bpftrace` — датчики на каждом этаже: показывает, куда реально пошёл
  посетитель, на каком этаже задержался, где его выбросили.
