# Сценарий 3: FRR, zebra и kernel FIB
...
После выполнения необходимо уметь показать один маршрут в FRR RIB и тот же маршрут в
...
не форвардятся. Знакомо? Причина в архитектуре FRR: протокол маршрутизации и установка
маршрута в kernel — это два разных процесса.

zebra выполняет роль FIB Manager — единой точки, через которую все протоколы
(IS-IS, static, BGP) программируют аппаратную таблицу пересылки. В нашем случае
аппаратная часть — это Linux kernel FIB.

- **IS-IS (isisd)** — вычисляет shortest path на основе LSDB, но не устанавливает
  маршруты в ядро. Его результат — информация в RIB, а не запись в FIB.
- **zebra** — получает маршруты от всех протоколов, выбирает лучшие по
  administrative distance и через netlink записывает их в kernel FIB.
- **Kernel FIB** — таблица, по которой реально пересылаются пакеты. Без zebra isisd
  вычислил бы shortest path, но пакеты не пошли бы — маршрут остался в RIB, не попав
  в FIB.

Проблема: маршрут может быть в RIB (FRR), но отсутствовать в FIB (kernel). Тогда пакет
молча дропается ядром, хотя `show ipv6 route` показывает всё корректно.
