...
После выполнения необходимо уметь объяснить, почему `ip route`, `pyroute2` и FRR `zebra`
...
добавил и кому верить? Ответ — netlink: это шина программирования (IPC-интерфейс) между
Control Plane и Data Plane.

Netlink — семейство протоколов ядра для обмена сообщениями между kernel и userspace.
В аппаратном маршрутизаторе аналогом был бы интерфейс между CPU (control plane)
и ASIC/FPGA (data plane). В Linux эту роль выполняет RTNETLINK — подсемейство netlink
для маршрутов, интерфейсов, адресов и соседей. И zebra, и iproute2 пишут в одну и ту же
FIB через один и тот же канал.
...
| Инструмент | Что показывает | Практический смысл |
|------------|----------------|---------------|
...
Инструмент `strace` позволяет осуществлять трассировку (перехват) netlink-сообщений.
Он покажет, какие именно сообщения zebra передаёт ядру, когда IS-IS сходится.
Протокол маршрутизации живёт выше — `strace` не знает про IS-IS, он видит только
netlink-сообщения от zebra к kernel.
...
Если вывод `ip monitor route` отличается по форматированию, это нормально. В артефактах важно
...
В артефактах отдельно укажите, какой маршрут появился только в kernel, а какой виден в FRR. Это
