...
Этот документ закрывает вводный уровень для студентов, которые раньше не работали с Linux
networking, Containerlab, FRR и SRv6. Его не нужно заучивать полностью перед первой ЛР. Лучше
читать разделы по мере прохождения курса и возвращаться к схемам при разборе вывода команд.
...
| Data plane | `eth1`, `eth2` | Передача учебного трафика между роутерами | Маршрутизация, FRR, SRv6 |

Management-сеть не является частью моделируемой операторской сети. Если ping идёт по `eth0`, он
не доказывает работоспособность маршрутизации r1-r2-r3. В лабораторных работах нас интересуют
...
|----------|--------------------|-----------------|
| End / `uN` | Обработать SID узла и продолжить forwarding | ЛР5-ЛР6 |
| End.X / `uA` | Переслать через конкретную adjacency | ЛР6, ЛР10 |
| End.DT6 | Деинкапсулировать IPv6 и сделать lookup в VRF | ЛР11 |
| End.DT4 | Деинкапсулировать IPv4 и сделать lookup в VRF | ЛР11 |

Если студент не может объяснить, какой behavior выполняет узел, он ещё не понимает SRv6-путь,
...
До ЛР8 вся лаборатория использует Linux kernel dataplane: маршруты ставятся в kernel FIB, а
...
Для каждой ЛР держите один и тот же шаблон мышления:
