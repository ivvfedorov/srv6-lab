...
Этот документ углубляет понимание SRv6 за пределы базовых лаб Сценарий 5–Сценарий 7. Если вы прошли
базовый стенд и умеете включать locator + IS-IS — этот материал закрывает пробелы до уровня
production-ready.

**Целевая аудитория**: прошли Сценарий 1–Сценарий 7, понимают разницу между control plane и data plane,
...
В нашем стенде locator = `/64` (B = 64). Оставшиеся 64 бита делятся между Function и Argument.
...
В нашем стенде IS-IS автоматически создаёт uN (End, Prefix-SID) и uA (End.X, Adjacency SID).
...
## Минимальный трек для перехода от Сценарий 7 к advanced
...
4. Выполнить **Сценарий 10** (SR Policy).
5. Прочитать разделы **5** (BGP L3VPN) и **6** (Flex-Algo).
6. Выполнить **Сценарий 11** (BGP SRv6 L3VPN).
